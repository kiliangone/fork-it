...
#st.page_link("forkit3.py", label="Home", icon="🏠")
#st.page_link("new_user.py", label="New User", icon="1️⃣")
#st.page_link("existing_user.py", label="Exsiting User", icon="2️⃣", disabled=True)
...
st.write(f'You selected: {selected_user_id}')
...
if submitted:
    ...
    def load_rankings_data():
        rankings_df = pd.read_pickle('data/sample_df.pkl')
        return rankings_df
    ...
    # Writing all the stuff on the page
